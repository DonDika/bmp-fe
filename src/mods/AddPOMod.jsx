...
const AddPOMod = ({ onClose, onPurchaseAdded }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (mrRes.data.success) {
          // Filter material requests yang memiliki item dengan status "pending"
          // const materialRequestsWithPendingItems = mrRes.data.data.filter(mr => 
          //   mr.items && mr.items.some(item => item.status === "pending")
          // );
          const filteredMR = mrRes.data.data.filter(mr => mr.status !== "proses");
          
          const options = filteredMR.map((mr) => ({
            value: mr.id,
            label: mr.no_mr,
          }));
  ...
  const fetchMaterialRequestItems = async (materialRequestId) => {
    try {
      ...
      if (response.data.success && response.data.data.items) {
        ...
        const pendingItems = response.data.data.items.filter(item => item.status === "pending" || item.status === "requested");
    ...
    if (value) {
      ...
      if (items.length > 0) {
        updatedMaterialRequests[mrIndex].items = [
          { material_request_item_id: "", supplier: "", quantity: 0, price: 0 }
        ];
      } else {
        updatedMaterialRequests[mrIndex].items = [];
      }
  ...
  const handleItemChange = (mrIndex, itemIndex, field, value) => {
    ...
    // If changing material_request_item_id, set default quantity from original item
    if (field === "material_request_item_id") {
      const materialRequestId = updatedMaterialRequests[mrIndex].material_request_id;
      ...
      if (selectedItem && selectedItem.originalItem) {
        // Set suggested quantity from the original item
        updatedMaterialRequests[mrIndex].items[itemIndex].quantity = selectedItem.originalItem.quantity;
      }
    }
  ...
  const addMaterialRequest = () => {
    setPoInfo((prev) => ({
      ...
      material_requests: [
        ...
        {
          material_request_id: "",
          items: [{ material_request_item_id: "", supplier: "", quantity: 0 }],
        },
      ],
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!dataToSubmit.created_by || dataToSubmit.material_request_ids.length === 0 || dataToSubmit.items.length === 0) {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl shadow-lg max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Tambah Purchase Order</h2>
        ...
        <div className="grid grid-cols-1 gap-4 mb-6">
          <div>
            ...
            <input type="text" value={userEmail} disabled className="w-full p-2 border border-gray-300 rounded-md" />
          </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={item.supplier}
                            onChange={(e) =>
                              handleItemChange(mrIndex, itemIndex, "supplier", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={item.price}
                            onChange={(e) =>
                              handleItemChange(mrIndex, itemIndex, "price", e.target.value)
                            }
                            className="w-full p-2 border border-gray-300 rounded-md"
